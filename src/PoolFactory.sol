...
contract PoolFactory {
    ...
    function createPool(address tokenAddress) external returns (address) {
        ...
        // @audit-g | we can prevent double calls to `IERC20.name()` by caching the result in a variable
        ...
        string memory liquidityTokenSymbol = string.concat(
            "ts",
            IERC20(tokenAddress).name() // @audit-l | this should be `IERC20.symbol()` no ??
        );
