...
contract TSwapPool is ERC20 {
    ...
    uint256 private swap_count = 0; // @audit-i | slot -> 5
    ...
    /// @dev The invariant of this function is that the ratio of WETH, PoolTokens, and LiquidityTokens is the same ⚠️
    ...
    function deposit(
        ...
        external
        // @audit-? | missing `revertIfDeadlinePassed(deadline)` modifier from the function
        ...
        returns (uint256 liquidityTokensToMint)
    {
        ...
        if (totalLiquidityTokenSupply() > 0) {
            // @audit-c | means this pool has liquidity in it before
            ...
            if (maximumPoolTokensToDeposit < poolTokensToDeposit) {
                // @audit-q | should this not be renamed to `...TooLow()`
            ...
            if (liquidityTokensToMint < minimumLiquidityTokensToMint) {
                // @audit-q | should this not be renamed to `...TooHigh()`
    ...
    function _addLiquidityMintAndTransfer(
        ...
        // @audit-h | `_mint` should come after `safeTransferFrom`
        _mint(msg.sender, liquidityTokensToMint);
        // @audit-m | the `wethToDeposit` and `poolTokensToDeposit` are in swapped order
        // @audit-l | should events not be emitted last?
        emit LiquidityAdded(msg.sender, poolTokensToDeposit, wethToDeposit);

        // Interactions
        // @audit-?? | no way to check if these calls were successful?
        ...
        // @audit-h | assert that pool balance has increased accordingly
    }
        ...
        revertIfZero(minPoolTokensToWithdraw)
    {
        ...
        // @audit-q | is it possible to manipulate the price by sending an ERC20 token to the contract directly?
